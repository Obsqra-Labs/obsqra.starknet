#!/bin/bash
# Deploy StrategyRouterV2 with STRK as asset_token and test functions
...
echo -e "${CYAN}═══════════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  Deploy StrategyRouterV2 (STRK + Test Functions) to Starknet Sepolia${NC}"
echo -e "${CYAN}═══════════════════════════════════════════════════════════════════${NC}"
...
echo -e "  Ekubo Positions: ${CYAN}${EKUBO_POSITIONS:-NOT SET}${NC} ${YELLOW}(Required for liquidity)${NC}"
...
CLASS_HASH_FILE="/opt/obsqra.starknet/deployments/v2_strk_test_class_hash.txt"

if [ -f "$CLASS_HASH_FILE" ]; then
    CLASS_HASH=$(cat $CLASS_HASH_FILE)
    echo -e "Using existing class hash: ${GREEN}$CLASS_HASH${NC}"
else
    # Use sncast for declaration (same as deploy-v2-eth.sh)
...
# Deploy
echo ""
echo -e "${YELLOW}Deploying StrategyRouterV2...${NC}"
...
echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  ✅ StrategyRouterV2 Deployed!${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════════════════${NC}"
echo ""
...
# Save deployment
DEPLOY_JSON="/opt/obsqra.starknet/deployments/sepolia-v2-strk-test.json"
cat > $DEPLOY_JSON << EOF
{
  "network": "sepolia",
...
echo ""
echo -e "${YELLOW}📋 Test Functions Available:${NC}"
echo -e "  - approve_token_for_testing(token, spender, amount)"
echo -e "  - test_jediswap_only(amount)"
echo -e "  - test_ekubo_only(amount)"
echo ""
