#!/usr/bin/env python3
"""
Deploy contracts using v1 transactions (compatible with RPC 0.8)
"""
...
async def main():
    print("=" * 60)
    print("🚀 Deploying Updated Contracts with V1 Transactions")
    print("=" * 60)
    ...
    # ==========================================
    # Step 1: Declare RiskEngine with v1 tx
    # ==========================================
    print("📝 Step 1: Declaring RiskEngine (v1 transaction)...")
    ...
    try:
        # Use v2 declare (Sierra + CASM)
        declare_result = await account.sign_declare_v2(
            compiled_contract=risk_engine_compiled,
            compiled_class_hash=casm_class_hash,
            max_fee=int(0.01 * 10**18),
        )
        
        # Send transaction
        declare_response = await client.send_transaction(declare_result)
        print(f"   Declare TX: {hex(declare_response.transaction_hash)}")
        
        # Wait for acceptance
        await client.wait_for_tx(declare_response.transaction_hash)
        
        # Get class hash from transaction
        risk_engine_class_hash = declare_result.class_hash
        
        print(f"✅ RiskEngine Class Hash: {hex(risk_engine_class_hash)}")
    except Exception as e:
        if "already" in str(e).lower() or "class already declared" in str(e).lower():
            print("   ⚠️  Class may already be declared, computing hash...")
    ...
    deploy_call = await Contract.deploy_contract_v1(
        account=account,
    ...
    # ==========================================
    print("📝 Step 3: Declaring StrategyRouterV2 (v1 transaction)...")
    ...
    try:
        router_declare = await account.sign_declare_v2(
            compiled_contract=router_compiled,
            compiled_class_hash=router_casm_hash,
            max_fee=int(0.01 * 10**18),
        )
        
        router_declare_response = await client.send_transaction(router_declare)
        print(f"   Declare TX: {hex(router_declare_response.transaction_hash)}")
        
        await client.wait_for_tx(router_declare_response.transaction_hash)
    ...
    router_deploy = await Contract.deploy_contract_v1(
        account=account,
    ...
    with open("/opt/obsqra.starknet/deployments/sepolia.json", "r") as f:
        sepolia_deployments = json.load(f)
    
    sepolia_deployments["contracts"]["RiskEngineOrchestration"] = {
        "address": hex(new_risk_engine),
        "classHash": hex(risk_engine_class_hash),
        "version": "orchestration-v1"
    }
    sepolia_deployments["contracts"]["StrategyRouterV2Orchestration"] = {
        "address": hex(new_router),
        "classHash": hex(router_class_hash),
        "version": "orchestration-v1"
    }
    
    with open("/opt/obsqra.starknet/deployments/sepolia.json", "w") as f:
        json.dump(sepolia_deployments, f, indent=2)
