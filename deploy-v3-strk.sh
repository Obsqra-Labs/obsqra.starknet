#!/bin/bash
# Deploy StrategyRouterV3 with STRK as asset_token, slippage protection, and yield accrual fixes
...
echo -e "${CYAN}═══════════════════════════════════════════════════════════════════${NC}"
echo -e "${CYAN}  Deploy StrategyRouterV3 (STRK + Slippage Protection) to Starknet Sepolia${NC}"
echo -e "${CYAN}═══════════════════════════════════════════════════════════════════${NC}"
...
EKUBO_POSITIONS="0x06a2aee84bb0ed5dded4384ddd0e40e9c1372b818668375ab8e3ec08807417e5"  # Sepolia Positions contract
...
echo -e "  Default Slippage: ${GREEN}1% swaps, 0.5% liquidity${NC}"
echo ""
...
if [ ! -f "target/dev/obsqra_contracts_StrategyRouterV3.contract_class.json" ]; then
    echo -e "${RED}❌ Build failed! Contract class not found.${NC}"
    echo -e "${YELLOW}Expected: target/dev/obsqra_contracts_StrategyRouterV3.contract_class.json${NC}"
    exit 1
fi
...
CLASS_HASH_FILE="/opt/obsqra.starknet/deployments/v3_strk_class_hash.txt"
...
if [ -f "$CLASS_HASH_FILE" ]; then
    ...
    DECLARE_OUTPUT=$(sncast --account $ACCOUNT_PROFILE declare \
        --contract-name StrategyRouterV3 \
        --network sepolia 2>&1)
...
# Deploy
echo ""
echo -e "${YELLOW}Deploying StrategyRouterV3...${NC}"
...
echo ""
echo -e "${GREEN}═══════════════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}  ✅ StrategyRouterV3 Deployed!${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════════════════${NC}"
echo ""
...
# Save deployment
DEPLOY_JSON="/opt/obsqra.starknet/deployments/sepolia-v3-strk.json"
cat > $DEPLOY_JSON << EOF
{
  "network": "sepolia",
  "version": "v3",
  ...
  "contracts": {
    "strategyRouterV3": "$CONTRACT_ADDRESS",
    "riskEngine": "$RISK_ENGINE",
    "daoConstraintManager": "$DAO_MANAGER"
  },
  "classHashes": {
    "strategyRouterV3": "$CLASS_HASH"
  },
  "configuration": {
...
echo -e "  ✅ Slippage protection (1% swaps, 0.5% liquidity by default)"
echo -e "  ✅ Fixed yield accrual (ekubo salt bug)"
echo -e "  ✅ Individual yield accrual functions"
echo -e "  ✅ update_slippage_tolerance() function"
echo ""
echo -e "${YELLOW}📋 Next Steps:${NC}"
echo -e "  1. Update frontend .env.local with new contract address"
echo -e "  2. Test slippage protection with different settings"
echo -e "  3. Test yield accrual functions"
